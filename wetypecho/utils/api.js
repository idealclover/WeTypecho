...
module.exports = {
  ...
  ParseStat: function(ori_list) {
    ...
    var disk =
      ...
      "GB";
    var disk_pr =
      ((ori_list["DiskTotal"] - ori_list["DiskFree"]) / ori_list["DiskTotal"]) *
      100;
    ...
    var mem =
      "已用:" +
      Math.round(ori_list["memRealUsed"] * 100) / 100 +
      "MB 空余:" +
      Math.round(ori_list["memRealFree"] * 100) / 100 +
      "MB";
    genlist.push(new statmember("内存", mem, true, ori_list["memRealPercent"]));
    //网卡
    netlist.push(
      new statmember("入网", this.speedparse(ori_list["NetInput"]), false, 0)
    );
    netlist.push(
      new statmember("出网", this.speedparse(ori_list["NetOut"]), false, 0)
    );
    var net =
      "出网:" +
      this.caculate_spped_in(ori_list["NetOut"]) +
      " 入网:" +
      this.caculate_spped_out(ori_list["NetInput"]);
